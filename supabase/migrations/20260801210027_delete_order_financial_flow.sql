...
BEGIN
  ...
  -- Reverte saldo bancário das movimentações geradas pela venda/baixa antes de apagá-las.
  FOR v_mov IN
  ...
  UPDATE public.cliente_creditos
     SET pedido_id = NULL
   WHERE pedido_id = _pedido_id;

  UPDATE public.cliente_indicacoes
     SET primeiro_pedido_id = NULL
   WHERE primeiro_pedido_id = _pedido_id;

  UPDATE public.chat_mensagens
     SET pedido_id = NULL
   WHERE pedido_id = _pedido_id;

  UPDATE public.chamadas_recebidas
     SET pedido_gerado_id = NULL
   WHERE pedido_gerado_id = _pedido_id;

  UPDATE public.vendas_antecipadas
     SET pedido_utilizacao_id = NULL
   WHERE pedido_utilizacao_id = _pedido_id;

  UPDATE public.vale_gas
     SET venda_id = NULL
   WHERE venda_id = _pedido_id;
  ...
  DELETE FROM public.boletos_emitidos
   WHERE conta_receber_id = ANY(v_contas_receber);

  DELETE FROM public.pagamentos_cartao
   WHERE conta_receber_id = ANY(v_contas_receber)
      OR pedido_id = _pedido_id;

  DELETE FROM public.devolucao_itens
   WHERE devolucao_id = ANY(v_devolucoes);
