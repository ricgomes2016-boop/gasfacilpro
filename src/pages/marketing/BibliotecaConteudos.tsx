...
import {
  Search, FileText, Image, Video, Star, StarOff, Trash2, Copy, Filter,
  Sparkles, Instagram, Facebook, Music2, MessageSquare,
} from "lucide-react";
  ...
  const filtered = conteudos.filter((c) => {
    ...
    if (busca) {
      ...
      return (
        ...
        c.conteudo?.toLowerCase().includes(search) ||
        c.hashtags?.toLowerCase().includes(search)
      );
    }
    return true;
  });
    ...
    <MainLayout>
      ...
      <div className="p-4 md:p-6 max-w-6xl mx-auto space-y-4">
        <div className="flex flex-wrap gap-3 items-center">
          ...
          <Button onClick={() => navigate("/clientes/marketing")} size="sm">
            <Sparkles className="h-4 w-4 mr-1" /> Criar Novo
          </Button>
        </div>
        ...
        {filtered.length === 0 ? (
          <Card className="border-dashed">
            <CardContent className="py-12 text-center">
              ...
              <p className="text-sm text-muted-foreground mb-4">
                Use o Marketing IA para gerar conteúdos automaticamente
              </p>
              <Button onClick={() => navigate("/clientes/marketing")}>
                <Sparkles className="h-4 w-4 mr-1" /> Criar Conteúdo
              </Button>
            </CardContent>
          </Card>
        ) : (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {filtered.map((c) => {
              const TipoIcon = tipoIcons[c.tipo] || FileText;
              return (
                  ...
                  <CardContent className="p-4 space-y-3">
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        {c.plataforma && (
                          <span className="text-sm">{plataformaIcons[c.plataforma] || ""}</span>
                        )}
                      </div>
                      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <Button
                          ...
                          size="icon"
                          className="h-7 w-7"
                          onClick={() => toggleFav.mutate({ id: c.id, favorito: !c.favorito })}
                        >
                        ...
                        <Button
                          ...
                          size="icon"
                          className="h-7 w-7"
                          onClick={() => c.conteudo && copyToClipboard(c.conteudo)}
                        >
                    ...
                    {c.titulo && <p className="font-medium text-sm">{c.titulo}</p>}
                    {c.conteudo && (
                      <p className="text-sm text-muted-foreground line-clamp-4">{c.conteudo}</p>
                    )}
                    {c.hashtags && (
                      <p className="text-xs text-primary/70 truncate">{c.hashtags}</p>
                    )}
