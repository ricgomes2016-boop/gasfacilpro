...
type Linha = { unidadeId: string | null; loja: string; produto: string; qtd: number; total: number; precoMedio: number };
...
export function ResumoPorLoja({ compras, unidadesMap }: Props) {
  const { linhas, totalQtd, totalValor } = useMemo(() => {
    const map = new Map<string, { unidadeId: string | null; loja: string; produto: string; qtd: number; total: number }>();
    compras.forEach((c) => {
      if (c.tipo_produto && c.tipo_produto !== "cheio") return;
      const produto = detectarProduto(c);
      if (!produto) return;
      const qtd = Number(c[`qtd_${produto.toLowerCase()}`] || 0);
      if (qtd <= 0) return;
      const unidadeId: string | null = c.unidade_id || null;
      const loja = unidadeId ? (unidadesMap.get(unidadeId) || "Sem filial") : "Sem filial";
      const key = `${unidadeId || "_"}__${produto}`;
      const r = map.get(key) || { unidadeId, loja, produto, qtd: 0, total: 0 };
      r.qtd += qtd;
    ...
    const linhas: Linha[] = Array.from(map.values())
      ...
      .sort((a, b) => a.loja.localeCompare(b.loja) || a.produto.localeCompare(b.produto));
    ...
    <Card className="border-border/40">
      <CardContent className="p-4 space-y-3">
        <div className="flex items-center gap-2">
          ...
          <p className="font-semibold text-sm text-foreground">Resumo por Loja — GLP Cheio</p>
        </div>
        {linhas.length === 0 ? (
          <p className="text-xs text-muted-foreground py-6 text-center">Sem compras de GLP cheio no período</p>
        ) : (
          <div className="overflow-x-auto rounded-lg border border-border/40">
            <table className="w-full text-xs">
              <thead className="bg-muted/50">
                <tr className="text-left">
                  {["Loja", "Produto", "Qtd", "Preço Médio Unit.", "Total Líquido"].map((h) => (
              ...
              <tbody className="divide-y divide-border/40">
                ...
                <tr className="bg-muted/40 font-bold">
                  <td className="px-3 py-2 text-foreground" colSpan={2}>Total Geral (GLP Cheio)</td>
