...
import {
  abrirLoginVenderGas,
  emitirDocumentoVenderGas,
  type VenderGasPayload,
} from "@/lib/fiscal/venderGasAgent";
...
export function EmitirNfeVenderGasDialog({
  pedido,
  open,
  onOpenChange,
  tipoDocumento,
  ...
  const abrirLogin = async () => {
    ...
    if (resposta.ok) toast.success(resposta.mensagem);
  ...
  const emitir = async () => {
    ...
    try {
      ...
      if (exigeDestinatario && !pedido.cliente_id)
        throw new Error("A NF-e exige um cliente cadastrado com CPF ou CNPJ.");
      let cliente: any = null;
      if (pedido.cliente_id) {
        const { data, error: clienteError } = await supabase
          .from("clientes")
          .select(
            "nome, razao_social, cpf, cnpj, inscricao_estadual, endereco, numero, bairro, cep, cidade, estado, codigo_municipio, telefone",
          )
          .eq("id", pedido.cliente_id)
          .single();
        if (clienteError && exigeDestinatario)
          throw new Error(
            "Não foi possível carregar os dados fiscais do cliente.",
          );
        cliente = data;
      }
      ...
      if (
        tipoDocumento === "nfce" &&
        documentoDigitado &&
        ![11, 14].includes(documentoDigitado.length)
      ) {
        throw new Error(
      ...
      if (exigeDestinatario && ![11, 14].includes(documento.length))
        throw new Error(
          "A NF-e exige CPF ou CNPJ válido no cadastro do cliente.",
        );
      const documentoFiscal = [11, 14].includes(documento.length)
        ? documento
        : "";
      if (
        exigeDestinatario &&
        ...
        throw new Error(
          "Complete endereço, número, CEP, cidade e UF no cadastro do cliente antes de emitir.",
        );
      }
      if (!pedido.itens.length)
        throw new Error("O pedido não possui itens para faturamento.");
      ...
      const { data: existente } = await db
        .from("notas_fiscais")
        .select("id, status, numero, chave_acesso")
        .eq("pedido_id", pedido.id)
        .eq("tipo", tipoDocumento)
        .eq("provedor", "vendergas")
        .neq("status", "cancelada")
        .maybeSingle();
      if (existente?.status === "autorizada")
        throw new Error(
          `Este pedido já possui a ${rotulo} ${existente.numero || existente.chave_acesso || "autorizada"}.`,
        );
      ...
      if (!notaId) {
        const { data: criada, error: criarError } = await db
          ...
          .insert({
            ...
            pedido_id: pedido.id,
            provedor: "vendergas",
            provedor_status: "aguardando_agente",
      ...
      if (!resposta.ok) {
        if (notaId)
          await db
            .from("notas_fiscais")
            .update({
              provedor_status: resposta.motivo || "falha",
              motivo_rejeicao: resposta.mensagem,
              integracao_resultado: resposta,
            })
            .eq("id", notaId);
      ...
      if (resposta.etapa === "pronta_para_revisao") {
        if (notaId)
          await db
            ...
            .update({
              status: "rascunho",
              provedor_status: "pronta_para_revisao",
              motivo_rejeicao: null,
              integracao_resultado: resposta,
            })
            .eq("id", notaId);
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Emitir {rotulo} no Vender Gás
          </DialogTitle>
          <DialogDescription>
            Pedido #{pedido?.numero_sequencial ?? pedido?.id.slice(0, 8)} ·{" "}
            {pedido?.cliente}
          </DialogDescription>
        </DialogHeader>
        <Alert>
          ...
          <AlertDescription>
            O agente abrirá a conta da Forte Gás e preencherá a{" "}
            <strong>{rotulo}</strong>, mas não transmitirá a nota. Você assumirá
            a janela para conferir cliente, itens e o valor de{" "}
            <strong>R$ {Number(pedido?.valor || 0).toFixed(2)}</strong>. Na
            NFC-e, CPF/CNPJ é opcional; na NF-e, é obrigatório.
          </AlertDescription>
        </Alert>
        {tipoDocumento === "nfce" && (
          <div className="space-y-2">
            <Label htmlFor="documento-nfce">
              CPF ou CNPJ do consumidor (opcional)
            </Label>
            <Input
              id="documento-nfce"
              inputMode="numeric"
            ...
            <p className="text-xs text-muted-foreground">
              Os demais dados do destinatário não são necessários para NFC-e.
            </p>
          </div>
        )}
        {erro && (
          <Alert variant="destructive">
            <AlertTitle>Emissão interrompida</AlertTitle>
