...
interface CircleAuthLayoutProps {
  ...
  /** Custom content for the gradient panel. Defaults to the portal quote. */
  sideContent?: ReactNode;
  /** Hide the compact mobile quote/header when the form should be the first focus. */
  showMobileHero?: boolean;
  /** Hide the fixed quote footer to avoid repeated decorative messaging. */
  showFooter?: boolean;
  /** Optional footer content when the quote footer is enabled. */
 ...
 * - Left: form (children)
 * - Right (md+): large gradient circle with contextual content
 * - Mobile: optional compact top header
 */
export function CircleAuthLayout({
  ...
  const gradientStyle = {
    background: `radial-gradient(circle at 30% 30%, hsl(${gradientFrom} / 0.9), hsl(${gradientTo}) 72%)`,
  } as React.CSSProperties;

  return (
    <div
      className={cn(
        "min-h-screen flex flex-col items-center justify-center p-3 md:p-6",
        showFooter ? "pb-20 md:pb-24" : "pb-3 md:pb-6",
    ...
    >
      ...
      >
        <div className="grid md:grid-cols-2">
          {showMobileHero && (
            <div
              className="md:hidden relative h-40 flex items-end justify-center px-6 pb-5 text-white overflow-hidden"
              style={gradientStyle}
            >
              <div
                className="absolute -top-24 -right-16 w-72 h-72 rounded-full opacity-20"
          ...
          <div className="hidden md:flex relative items-center justify-center overflow-hidden min-h-[520px]">
            ...
            />
            <div
              className="absolute -right-10 top-10 w-24 h-24 rounded-full border opacity-20"
              style={{ borderColor: `hsl(${gradientFrom} / 0.5)` }}
            />
            <div
              className="absolute right-40 bottom-12 w-12 h-12 rounded-full border opacity-30"
              style={{ borderColor: `hsl(${gradientTo} / 0.6)` }}
            />

            <div className="relative z-10 text-white max-w-sm px-8 animate-fade-in">
              {sideContent ?? (
