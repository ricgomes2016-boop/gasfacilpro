    ...
    <>
      ...
      {open && (
        <div className={`fixed inset-x-0 bottom-0 sm:inset-auto sm:bottom-6 sm:right-6 z-50 sm:w-[380px] h-[80vh] sm:h-[560px] border sm:rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom duration-200 ${
          ...
          {/* Header */}
          <div
            className={`relative px-4 py-3 flex items-center gap-3 overflow-hidden ${lightTheme ? "border-b-[3px] border-[#E8620C]" : `bg-gradient-to-r ${gradient}`}`}
            style={lightTheme ? { backgroundColor: "#0F2A4A", color: "#ffffff" } : undefined}
          >
            {!lightTheme && <div className="absolute inset-0 opacity-30 pointer-events-none"
              ...
              >
                <div
                  className={`max-w-[85%] px-3.5 py-2.5 rounded-2xl text-sm whitespace-pre-wrap leading-relaxed ${
                    m.role === "user"
                      ? lightTheme
                        ? "bg-[#0F2A4A] text-white border border-[#0F2A4A] shadow-sm rounded-br-sm"
                  ...
                  style={m.role === "user" && lightTheme ? { color: "#ffffff" } : undefined}
                >
                  {m.content}
                </div>
              </div>
          ...
          <div className={`border-t p-3 ${lightTheme ? "border-[#D8E0EA] bg-white" : "border-white/10 bg-[#0a0118]"}`}>
            <div className="flex gap-2 items-center">
              ...
              <button
                ...
                className={`w-11 h-11 shrink-0 rounded-full text-white flex items-center justify-center disabled:opacity-40 hover:scale-105 transition-transform ${lightTheme ? "bg-[#E8620C]" : `bg-gradient-to-br ${gradient}`}`}
                style={{ color: "#ffffff" }}
                aria-label="Enviar"
              >
